...
STORE filtered_products INTO 'warehouse/pig/output/q1' USING PigStorage(',');
...
STORE filtered_orders INTO 'warehouse/pig/output/q2' USING PigStorage(',');

-- 3
customer_names = FOREACH customers GENERATE customer_name;
STORE customer_names INTO 'warehouse/pig/output/q3' USING PigStorage(',');

-- 4
filtered_customers = FILTER customers BY SUBSTRING(customer_name, 0, 1) == 'J';
STORE filtered_customers INTO 'warehouse/pig/output/q4' USING PigStorage(',');
...
STORE aggregated_data INTO 'warehouse/pig/output/q5' USING PigStorage(',');
...
STORE result INTO 'warehouse/pig/output/q6' USING PigStorage(',');

-- 7
joined_tables_filtered = JOIN customers BY (product_id, order_date), orders BY (product_id, order_date);
STORE joined_tables_filtered INTO 'warehouse/pig/output/q7' USING PigStorage(',');
